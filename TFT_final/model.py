...
class TemporalFusionTransformer(nn.Module):
    ...
    def forward(self, 
        ...
        static_input = self.static_emb(torch.tensor(static_input, dtype=int)) # (batch,1,static_num,d_model)
        future_input = self.future_emb(torch.tensor(future_input, dtype=int)) # (batch,decoder_len,future_num,d_model)
        category_input = self.category_emb(torch.tensor(category_input, dtype=int)) # (batch,encoder_len,cate_input_num,d_model)
